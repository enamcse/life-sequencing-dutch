...
def generate_plots(df: pd.DataFrame, stats_dir: Path):
    ...
    if 'death_date' in deaths_df.columns and deaths_df['death_date'].notna().any():
        ...
        # Add value labels on bars (only for finite values)
        max_val = yearly_deaths.max() if len(yearly_deaths) > 0 else 1
        for i, v in enumerate(yearly_deaths.values):
            if np.isfinite(v) and np.isfinite(max_val):
                ax.text(i, v + max_val * 0.01, f'{v:,}', ha='center', fontsize=11, fontweight='bold')
    ...
    if 'age_at_death' in deaths_df.columns and deaths_df['age_at_death'].notna().any():
        ...
        max_val = age_group_counts.max() if len(age_group_counts) > 0 else 1
        for i, v in enumerate(age_group_counts.values):
            if np.isfinite(v) and np.isfinite(max_val):
                ax.text(i, v + max_val * 0.01, f'{v:,}', ha='center', fontsize=10)
    ...
    # All persons - drop NaN values for histogram
    ax1 = axes[0]
    birth_years_clean = df['birth_year'].dropna()
    if len(birth_years_clean) > 0:
        birth_years_clean.hist(bins=50, ax=ax1, color='#3498db', alpha=0.7, edgecolor='black')
    ...
    # By label - drop NaN values for histogram
    ax2 = axes[1]
    alive_birth_years = df[df['is_dead'] == 0]['birth_year'].dropna()
    dead_birth_years = df[df['is_dead'] == 1]['birth_year'].dropna()
    if len(alive_birth_years) > 0:
        alive_birth_years.hist(bins=50, ax=ax2, alpha=0.6, label='Alive', color='#2ecc71')
    if len(dead_birth_years) > 0:
        dead_birth_years.hist(bins=50, ax=ax2, alpha=0.6, label='Dead', color='#e74c3c')
    ...
    alive_age_cutoff = df[df['is_dead'] == 0]['age_at_cutoff'].dropna()
    dead_age_cutoff = df[df['is_dead'] == 1]['age_at_cutoff'].dropna()
    if len(alive_age_cutoff) > 0:
        alive_age_cutoff.hist(bins=50, ax=ax, alpha=0.6, label='Alive', color='#2ecc71')
    if len(dead_age_cutoff) > 0:
        dead_age_cutoff.hist(bins=50, ax=ax, alpha=0.6, label='Dead', color='#e74c3c')
    ...
    mortality_by_age['mortality_rate'] = mortality_by_age['mortality_rate'].fillna(0)
    ...
    mortality_rate_clean = mortality_by_age['mortality_rate'].fillna(0)
    mortality_rate_clean.plot(kind='bar', ax=ax4, color='#e74c3c', alpha=0.8)
    ...
    alive_birth = df[df['is_dead'] == 0]['birth_year'].dropna()
    dead_birth = df[df['is_dead'] == 1]['birth_year'].dropna()
    if len(alive_birth) > 0:
        alive_birth.hist(bins=30, ax=ax5, alpha=0.6, label='Alive', color='#2ecc71')
    if len(dead_birth) > 0:
        dead_birth.hist(bins=30, ax=ax5, alpha=0.6, label='Dead', color='#e74c3c')
    ...
    # Safely compute statistics with NaN handling
    age_mean = df['age_at_cutoff'].mean()
    age_median = df['age_at_cutoff'].median()
    age_min = df['age_at_cutoff'].min()
    age_max = df['age_at_cutoff'].max()
    ...
    Age Statistics (at cutoff):
      • Mean: {age_mean:.1f} years
      • Median: {age_median:.1f} years
      • Min: {age_min:.1f} years
      • Max: {age_max:.1f} years
    """
    
    if 'age_at_death' in deaths_df.columns and deaths_df['age_at_death'].notna().any():
        death_age_mean = deaths_df['age_at_death'].mean()
        death_age_median = deaths_df['age_at_death'].median()
        if np.isfinite(death_age_mean) and np.isfinite(death_age_median):
            stats_text += f"""
    Age at Death Statistics:
      • Mean: {death_age_mean:.1f} years
      • Median: {death_age_median:.1f} years
    """
