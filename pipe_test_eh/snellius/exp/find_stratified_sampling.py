...
def main():
    ...
    output_dir = "~/pipe_test_eh/snellius/exp/find_stratified_sampling_output"
    ...
    print(f"✅ Computing density scores with k={k_neighbors} nearest neighbors...")
    density_scores = nearest_nei_dist_mean(embeddings, k=k_neighbors)
    df["density_score"] = density_scores

    # -------- Bin people into density buckets --------
    print(f"✅ Bucketing into {num_buckets} quantile bins...")
    df["bucket"] = pd.qcut(df["density_score"], q=num_buckets, labels=False, duplicates='drop')
    print(df["bucket"].value_counts().sort_index())

    # -------- Sample proportionally from buckets --------
    print(f"✅ Sampling {sample_size} people proportionally from buckets...")
    sampled_df = sample_proportionally_from_buckets(df, "bucket", sample_size)

    for variable in variables_to_compare:
        # -------- Compare distribution --------
        print(f"\n✅ Comparing metadata distribution for: {variable}")
