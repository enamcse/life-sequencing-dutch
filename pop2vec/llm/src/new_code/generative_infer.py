...
def _select_prefix_item(ds, prefix_id: Optional[str]):
    if prefix_id is None or str(prefix_id).strip() == "":
        logger.info("prefix_id not provided; using ds[0]")
        return ds[0]
...
def main():
    ...
    item = _select_prefix_item(ds, prefix_id)
    ...
    # 7) Pretty print originals (stdout + optional file)
    original_tokens = x4[0].tolist()
    pretty_print_tokens("ORIGINAL PREFIX TOKENS", original_tokens, vocab_df, with_category=with_category, out_path=tokens_write_path)

    # 8) Generate + pretty print generated
    ...
    pretty_print_tokens("GENERATED TOKENS", generated_tokens, vocab_df, with_category=with_category, out_path=tokens_write_path)
