...
def _train_target(
    ...
    # Compute metrics for reporting (unchanged)
    val_metrics = (
    ...
    # >>> Add this block to persist the chosen threshold for binary
    if target_type == "binary":
        probas = 1 / (1 + np.exp(-logits_all))
        best_thr, _ = _best_f1_threshold_torch(
            torch.from_numpy(probas), torch.from_numpy(y_val)
        )
        with torch.no_grad():
            if "best_thr" not in dict(model.named_buffers()):
                model.register_buffer("best_thr", torch.full((1,),float("nan"), device=next(model.parameters()).device))
            model.best_thr.copy_(best_thr.reshape_as(model.best_thr).to(model.best_thr.device))
        logging.info(f"[SAVE] best_thr = {float(model.best_thr):.4f}")

    # Re-snapshot state so the buffer is included with the final value
...
def _eval_test(
    ...
    if not hasattr(model, "best_thr"):
        model.register_buffer("best_thr", torch.tensor(float("nan"), device=device))
    ...
    if target_type == "binary":
        ...
        # (ensure _eval_test receives `test_df` that still has PRIMARY_KEY)
        rin = test_df[PRIMARY_KEY].to_numpy()
        pd.DataFrame({"RINPERSOON": rin, "prediction": probas}).to_csv(prob_path, index=False)
        pd.DataFrame({"RINPERSOON": rin, "prediction": preds}).to_csv(label_path, index=False)
